...
def _gh_post(url, payload_dict, token, method="POST"):
    payload = json.dumps(payload_dict).encode("utf-8")
    req = _urllib_req.Request(url, data=payload, headers=_GH_HEADERS(token), method=method)
    with _urllib_req.urlopen(req, timeout=15) as resp:
        return json.loads(resp.read().decode("utf-8"))


def _gh_get(url, token):
    req = _urllib_req.Request(url, headers={k: v for k, v in _GH_HEADERS(token).items() if k != "Content-Type"})
...
def _post_github_issue_async(title, body):
    ...
    try:
        _gh_post(
            ...
            {"title": title[:200], "body": body[:3000], "labels": ["bug", "production", "auto-report"]},
            token,
        )
...
def _post_github_pr_sync(error_title, endpoint, error_str, ts_str, token):
    """Create branch + incident file + PR stub. Returns (pr_number, pr_url)."""
    safe_ep = "".join(c if c.isalnum() or c == "-" else "-" for c in endpoint.strip("/").replace("/", "-"))[:40]
    ts_slug = ts_str[:16].replace(":", "-").replace("T", "-")
    branch = f"auto-incident/{ts_slug}-{safe_ep}"

    # 1. Get main SHA
    ref = _gh_get(f"https://api.github.com/repos/{_GITHUB_REPO}/git/refs/heads/main", token)
    ...
    # 3. Create incident file on the new branch
    incident_md = (
        f"# Incident Report — {ts_str[:19]}\n\n"
        f"**Auto-generated by Trade Surveillance Engine v2.0**\n\n"
        f"## Summary\n\n"
        f"| Field | Value |\n|-------|-------|\n"
        f"| Endpoint | `{endpoint}` |\n"
        ...
        f"- [ ] Fix implemented and pushed to this branch\n"
        f"- [ ] Tested on staging\n"
        f"- [ ] PR reviewed and merged\n\n"
        f"---\n*Auto-raised by self-healing system. Push fix to this branch, then merge.*\n"
    )
    file_content = _base64.b64encode(incident_md.encode("utf-8")).decode("utf-8")
    _gh_post(
        f"https://api.github.com/repos/{_GITHUB_REPO}/contents/incidents/incident-{ts_slug}.md",
        {
            "message": f"auto: incident report {ts_slug} ({safe_ep})",
    ...
    pr = _gh_post(
        ...
        {
            "title": f"🚨 Auto-incident: {error_title[:72]}",
            "body": (
                f"## Auto-Detected Production Failure\n\n"
                f"This PR was automatically created by the Trade Surveillance Engine's "
                f"self-healing system when a 500 error was detected.\n\n"
                f"| Field | Value |\n|-------|-------|\n"
                f"| **Endpoint** | `{endpoint}` |\n"
                f"| **Error** | {error_title[:120]} |\n"
                f"| **Detected** | {ts_str} |\n"
                f"| **Branch** | `{branch}` |\n\n"
                f"## What To Do\n\n"
                f"1. Review the incident report in `incidents/` on this branch\n"
                f"2. Push a fix to **this branch** (`{branch}`)\n"
                f"3. Get the PR reviewed and merge to `main`\n\n"
                f"## Checklist\n\n"
                f"- [ ] Root cause identified\n"
                f"- [ ] Fix pushed to this branch\n"
                f"- [ ] Tested\n"
                f"- [ ] Ready to merge\n\n"
                f"---\n"
                f"🤖 *Auto-raised by Trade Surveillance Engine v2.0 self-healing system*"
            ),
            "head": branch,
            "base": "main",
        },
...
def _raise_github_pr(error_title, endpoint, error_str):
    def _worker():
        token = os.getenv("GITHUB_TOKEN", "")
        if not token:
            return
        try:
            ...
            _post_github_pr_sync(error_title, endpoint, error_str, ts, token)
        except Exception as _e:
            logging.error(f"GitHub PR creation failed: {_e}")
...
def handle_500(error):
    try:
        ...
        err_str = str(error)
        _raise_github_issue(
            f"[{ts[:10]}] Production 500 Error — {endpoint}",
            f"**Auto-generated incident report**\n\n"
            f"**Endpoint:** `{request.method} {endpoint}`\n"
            f"**Timestamp:** {ts}\n"
            f"**Error:** {err_str}\n\n"
            f"Check Render logs at https://dashboard.render.com",
        )
        _raise_github_pr(f"500 on {endpoint} — {err_str[:60]}", endpoint, err_str)
    except Exception:
        pass
...
def github_test_issue():
    ...
    try:
        result = _gh_post(
            f"https://api.github.com/repos/{_GITHUB_REPO}/issues",
            {"title": title, "body": body, "labels": ["test", "auto-report"]},
            token,
        )
        return jsonify({
            "success": True,
...
def github_create_pr():
    ...
    try:
        ts = datetime.now(timezone.utc).isoformat()
        pr_number, pr_url = _post_github_pr_sync(
            "Manual test from Config page",
            "/api/test-endpoint",
            "This is a manually triggered test PR.\nNo real error occurred — testing the auto-PR self-healing system.",
            ts,
            token,
        )
        return jsonify({"success": True, "pr_number": pr_number, "pr_url": pr_url})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
